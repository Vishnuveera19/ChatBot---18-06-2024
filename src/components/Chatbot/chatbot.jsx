...
function Chatbot() {
  ...
  const handleButtonClick = (text) => {
    ...
    switch (text.toLowerCase()) {
      ...
      case 'leave':
        botResponse = (
          <div>
            <Button variant="outlined" onClick={() => handleBranchOption('Today')} sx={{ m: 0.5 }}>
              Today
            </Button>
            <Button variant="outlined" onClick={() => handleBranchOption('Week')} sx={{ m: 0.5 }}>
              Week
            </Button>
            <Button variant="outlined" onClick={() => handleBranchOption('Month')} sx={{ m: 0.5 }}>
              Month
            </Button>
          </div>
        );
  ...
  const handleBranchOption = (option) => {
      ...
      <FormControl fullWidth>
        ...
        <select
          ...
          onChange={(e) => {
            ...
            fetchLeaveData(e.target.value, option);
          }}
          style={{ height: '50px' }}
          inputlabelprops={{ shrink: true }}
        >
    ...
    setTimeout(() => {
      const replyMessage = { text: botResponse, sender: 'bot', branchDropdown };
      setMessages((prevMessages) => [...prevMessages, replyMessage]);
      scrollToBottom();
    }, 500);
  };
  ...
  return (
    <ThemeProvider theme={theme}>
      <Box className="chatbot-container" sx={{ display: 'flex', flexDirection: 'column', height: '90vh', width: '20%', p: 1 }}>
        <Box className="chatbot-header" sx={{ p: 2, backgroundColor: '#1976d2', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderRadius: '8px 8px 0 0' }}>
          <Typography variant="h6">chatbot! 👋</Typography>
        </Box>

        <Paper className="chatbot-messages" sx={{ p: 2, mb: 2, flex: 1, overflowY: 'auto', borderRadius: '0 0 8px 8px' }}>
